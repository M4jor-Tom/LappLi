...
class MaterialResourceIT {
    ...
    @Test
    @Transactional
    void getAllMaterialsByMaterialMarkingStatisticsIsEqualToSomething() throws Exception {
        // Initialize the database
        materialRepository.saveAndFlush(material);
        MaterialMarkingStatistic materialMarkingStatistics;
        if (TestUtil.findAll(em, MaterialMarkingStatistic.class).isEmpty()) {
            materialMarkingStatistics = MaterialMarkingStatisticResourceIT.createEntity(em);
            em.persist(materialMarkingStatistics);
            em.flush();
        } else {
            materialMarkingStatistics = TestUtil.findAll(em, MaterialMarkingStatistic.class).get(0);
        }
        em.persist(materialMarkingStatistics);
        em.flush();
        material.addMaterialMarkingStatistics(materialMarkingStatistics);
        materialRepository.saveAndFlush(material);
        Long materialMarkingStatisticsId = materialMarkingStatistics.getId();

        // Get all the materialList where materialMarkingStatistics equals to materialMarkingStatisticsId
        defaultMaterialShouldBeFound("materialMarkingStatisticsId.equals=" + materialMarkingStatisticsId);

        // Get all the materialList where materialMarkingStatistics equals to (materialMarkingStatisticsId + 1)
        defaultMaterialShouldNotBeFound("materialMarkingStatisticsId.equals=" + (materialMarkingStatisticsId + 1));
    }
